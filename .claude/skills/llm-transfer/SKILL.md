...
## 1. Role
You are {persona - e.g. a senior Python engineer / a Flask reviewer / a pandas tutor}.
...
situation: what the project is (a preserved Python bootcamp repo - a Flask joke/country web
app plus Jupyter notebooks), the domain, and WHY this task matters. Self-contained - assume
the reader has never seen this codebase.}
...
## 6. Constraints & Rules
- Tech stack: {Python 3 via uv (no requirements.txt - deps injected per-run with
  `uv run --with ...`), Flask + requests, Jupyter notebooks; run via `just`}
...
- **Selective, not a dump.** Include only artifacts that bear on the task; summarize the rest.
  ...
  windows can be small). In this repo `WebApp/script.py` + the touched template(s) usually
  fit whole; for a notebook, inline the relevant code cells, never the raw `.ipynb` JSON.
...
3. **Use our skills:** name the relevant playbook(s) and tell it to follow them - _"For this
   ...
   its scripts, e.g. `uv run --no-project python .claude/skills/<skill>/<x>.py`."_ Our skills are just markdown
...
Target: _"Hand off a form-input bug to an external LLM for a fix."_

````text
# MASTER PROMPT - Guard the joke-amount field against non-numeric input

## 1. Role
You are a senior Python/Flask engineer who writes precise, minimal fixes.

## 2. Mission
Stop the `/specific_joke` route from crashing when the form's amount field is not a number,
without changing any other route's behavior.

## 3. Background & Context
Preserved Python bootcamp project: a small Flask app that fetches jokes from JokeAPI and
renders them with Jinja2 templates. No repo access - everything you need is below.

## 4. Current State
- Done: the route works end-to-end with well-formed input.
- Failing: a non-numeric amount raises the exception below and Flask returns a 500.
...
## 6. Constraints & Rules
- Flask + stdlib only, no new dependencies. Errors must render the existing `error.html`
  via `render_template('error.html', error_message=...)`, matching the app's pattern.

## 7. Your Task
Return the corrected read (reject non-numeric input with the error page) and a one-paragraph
explanation.

## 8. Output Format & Success Criteria
- Output: the fixed snippet only + the explanation.
- Done: non-numeric input renders the error page; numeric input still works.

## 9. Open Questions & Assumptions
- Assume negative amounts keep their current behavior (this fix targets only the crash).

## Reminder
Return the fixed amount-field read + a one-paragraph explanation. No other behavior changes.
````
...
- Ported from marks-counter for python-bootcamp-projects - same 9-section framework and
  dual-mode (agentic brief vs cold master prompt); personas, worked example, and stack
  references adapted to the Flask app + notebooks + uv stack.
