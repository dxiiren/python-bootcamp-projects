---
name: lint-check
description: Use when the developer says 'lint check', 'run lint', 'check lint', 'run the quality suite', or 'lint everything' — runs the quality checks available to this repo (Python syntax gate via py_compile, Flask boot smoke on port 8124, placeholder + debug-leftover greps) and reports pass/fail per layer. No flake8/ruff/pytest here — the repo has no lint toolchain on purpose.
model: sonnet
---

# lint-check — Quality suite (py_compile · Flask boot smoke · greps)

This repo has **no linter, no formatter, no tests** — it is a preserved bootcamp project
with no dependency manifest (every run injects deps via `uv run --with ...`). The honest
quality layers are the Python compiler's syntax check, a boot-to-200 smoke of the Flask
app, and two greps. Run each independently so one failure doesn't hide the others.
...
- Don't bolt on ruff/flake8/pytest uninvited — this is a preserved bootcamp project;
