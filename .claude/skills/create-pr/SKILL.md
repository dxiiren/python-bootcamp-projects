...
`github.com/dxiiren/python-bootcamp-projects`.
...
- If there are multiple commits, synthesize one Conventional title that summarizes
  the change, e.g. `feat(webapp): migrate country lookup off REST Countries v3.1` or
  `docs(tooling): document the shared 8124 port rule`. Pick the type/scope from the
...
- <how it was verified: `py_compile` exit code, `just serve` + GET `/` http code,
  `just execute '<nb>'` result, manual `just lab` cell run, etc.>
```
...
title used. This repo has no CI — verification is the syntax-gate / boot-smoke
...
| `/pre-pr-review` | Optional self-review (Flask + notebooks checklist) before opening the PR      |
