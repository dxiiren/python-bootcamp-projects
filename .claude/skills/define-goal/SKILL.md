...
4. **Guardrails & locked decisions** - may it `git commit` / `push` / stage? May it open a PR /
   ...
   **locked** (do-not-relitigate)? Any **environment bootstrap** (a sanity gate like
   `py_compile` exiting 0 and `just serve` answering 200 on 8124)?
...
- Ported from marks-counter for python-bootcamp-projects - same interrogation ->
  confirm-until-100% -> stop-proof goal-file flow and the five stop-proof pillars;
  bootstrap/verification examples adapted to the uv/Flask/notebook stack
  (`py_compile` + `just serve` smoke; no deploy targets here).
