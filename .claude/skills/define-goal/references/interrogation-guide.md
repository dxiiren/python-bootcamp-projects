...
- What's the before->after transformation, stated concretely enough to apply uniformly (e.g.
  raw `request.form[...]` reads -> a validated read helper, or a naming convention pass)?
- Isolation: do items touch shared files (-> serialize) or disjoint ones (-> can parallelize)?
- What's the per-item verification (`py_compile` exit 0 / `just serve` + a `curl.exe` probe of
  the touched route / grep) that proves the change is safe?

### Bugfix batch

- Is each bug independently reproducible? What's the repro + the fix-verified check per bug?
  (For this app, a repro is usually a `curl.exe` GET/POST against the running `just serve`.)
