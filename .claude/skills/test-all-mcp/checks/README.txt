MCP smoke-test prompts -- one .txt per server, for python-bootcamp-projects.
...
FILES (3 server checks)
  ...
  Browser:
    playwright.txt       -> browser automation (about:blank, or :8124 while `just serve` runs) [turnkey]
  Repo (per-dev):
