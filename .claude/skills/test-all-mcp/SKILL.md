...
- **playwright** — the first call may need the Chromium binary (`npx playwright install chromium`);
  a slow first launch is not a hang. Navigate to `about:blank` for a self-contained smoke — the
  Flask app on `http://127.0.0.1:8124` only exists while `just serve` is running.
