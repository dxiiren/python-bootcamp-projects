...
5. **Review in plan mode**, then on approval (create the folder if missing):
   - Write `.claude/workspace/reports/transfers/claude/{YYYY-MM-DD}-{topic}.md`, where
     `{topic}` names **what the session did** (e.g. `migrate-country-lookup-api`), **not**
     this skill's name.
...
3. **Re-ground before acting** (do NOT start new work first): read the brief's pointers + the
   recent `git log`, then run a **basic functional check**
   (`uv run --no-project python -m py_compile WebApp/script.py` must exit 0; if the session's
   work touches the web app, also boot `just serve` and GET `/` for a 200, then `just stop`)
...
- Files: `WebApp/script.py:105-130`, `WebApp/templates/country.html`
- Recent commits: `<hash> <subject>` (run `git log --oneline -5`)
- Branch: {branch} PR: #{id}
- Commands: `just serve`, `just lab`, `just execute '<nb>'`, `just stop`
...
- Templates in sync with the route variables they render: {yes/no as of close}
- No server left running on 8124: {yes/no} - `just stop` if unsure
...
- Ported from marks-counter for python-bootcamp-projects - same pointer-based, opt-in-resume
  design; commands + freshness checks adapted to the Flask/notebook `just serve`/`just lab`
  stack (including the shared-port-8124 check).
